...
function getReminderLabel(type, day) {
  switch(type) {
    ...
    case 'weekly': return `毎週${['日','月','火','水','木','金','土'][day]}曜`;
    case 'monthly': return '毎月';
    case 'monthly_date': return `毎月${day}日`;
    default: return '';
  }
}
...
async function render() {
  ...
  app.innerHTML = `
    ...
    <!-- サイドバーオーバーレイ -->
    <div class="sidebar-overlay fixed inset-0 bg-black/50 z-90 ${state.sidebarOpen ? '' : 'hidden'}" 
    ...
    <aside class="sidebar ${state.sidebarOpen ? 'open' : ''}">
      ...
      <nav>
        <!-- スマートリスト -->
        <div class="sidebar-item ${state.currentView === 'today' ? 'active' : ''}" 
             onclick="setView('today')">
          <i class="fas fa-sun text-yellow-400"></i>
          <span>今日</span>
          ${state.stats.today ? `<span class="badge">${state.stats.today}</span>` : ''}
          ${state.stats.overdue ? `<span class="badge bg-red-500/50">${state.stats.overdue}</span>` : ''}
        </div>
        
        <div class="sidebar-item ${state.currentView === 'upcoming' ? 'active' : ''}" 
             onclick="setView('upcoming')">
          <i class="fas fa-calendar text-blue-400"></i>
          <span>予定</span>
          ${state.stats.upcoming ? `<span class="badge">${state.stats.upcoming}</span>` : ''}
        </div>
        
        <div class="sidebar-item ${state.currentView === 'important' ? 'active' : ''}" 
             onclick="setView('important')">
          <i class="fas fa-star text-yellow-500"></i>
          <span>重要</span>
          ${state.stats.important ? `<span class="badge">${state.stats.important}</span>` : ''}
        </div>
        
        <div class="sidebar-item ${state.currentView === 'all' ? 'active' : ''}" 
             onclick="setView('all')">
        ...
        ${state.sections.map(s => `
          <div class="sidebar-item ${state.currentView === 'section' && state.currentSectionId == s.id ? 'active' : ''}" 
        ...
        <div class="mt-4 border-t border-gray-700 pt-4">
          <div class="sidebar-item ${state.currentView === 'trash' ? 'active' : ''}" 
               onclick="setView('trash')">
            <i class="fas fa-trash text-gray-500"></i>
            <span>ゴミ箱</span>
            ${state.stats.trash ? `<span class="badge">${state.stats.trash}</span>` : ''}
          </div>
        </div>
      </nav>
    </aside>
...
function renderTasks() {
  if (state.tasks.length === 0) {
    return `
      <div class="empty-state">
        <i class="fas fa-${state.currentView === 'trash' ? 'trash' : 'check-circle'}"></i>
        <p>${state.currentView === 'trash' ? 'ゴミ箱は空です' : 'タスクがありません'}</p>
      </div>
    `;
  }
  ...
  return state.tasks.map(task => {
    ...
    return `
      <div class="task-card ${task.is_completed ? 'completed' : ''} ${isOverdue ? 'overdue' : ''} ${isToday ? 'today' : ''}"
           onclick="showTaskDetail(${task.id})">
        <div class="flex items-start gap-3">
          ${state.currentView !== 'trash' ? `
            <div class="checkbox ${task.is_completed ? 'checked' : ''} ${task.is_important ? 'important' : ''}"
                 onclick="event.stopPropagation(); toggleComplete(${task.id}, ${!task.is_completed})">
              ${task.is_completed ? '<i class="fas fa-check text-white text-sm"></i>' : ''}
            </div>
          ` : ''}
          
          <div class="flex-1 min-w-0">
            <h3 class="font-medium ${task.is_completed ? 'line-through text-gray-500' : ''}">${escapeHtml(task.title)}</h3>
            ...
            <div class="flex items-center gap-2 mt-2 flex-wrap">
              ...
              ${task.due_date ? `
                <span class="due-badge ${dueClass}">
                  <i class="fas fa-calendar-alt mr-1"></i>${formatDate(task.due_date)}
                </span>
              ` : ''}
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <select id="task-section" class="input">
                  ...
                  ${state.sections.map(s => `
                    <option value="${s.id}" ${state.currentSectionId == s.id ? 'selected' : ''}>
                      ${s.icon} ${s.name}
                    </option>
                  `).join('')}
                </select>
              ...
              <div>
                ...
                <input type="date" id="task-due" class="input" 
                       value="${state.currentView === 'today' ? new Date().toISOString().split('T')[0] : ''}">
              </div>
            </div>
...
function updateReminderDay() {
  ...
  if (type === 'weekly') {
    ...
    select.innerHTML = ['日','月','火','水','木','金','土'].map((d, i) => 
      `<option value="${i}">${d}曜日</option>`
    ).join('');
...
async function showTaskDetail(id) {
  ...
  modal.innerHTML = `
    <div class="modal-overlay" onclick="closeModal()">
      <div class="modal-content" onclick="event.stopPropagation()">
        <div class="modal-header">
          <h3 class="text-lg font-bold">${isTrash ? 'ゴミ箱のタスク' : 'タスク詳細'}</h3>
          <button onclick="closeModal()" class="text-gray-400 hover:text-white">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <div class="modal-body">
          ${isTrash ? `
            <div class="mb-4">
              ...
              ${task.description ? `<p class="text-gray-400 mt-2">${escapeHtml(task.description)}</p>` : ''}
            </div>
            <div class="flex gap-3">
          ...
          ` : `
            <form onsubmit="updateTask(event, ${id})">
              ...
              <div class="mb-4">
                <textarea id="edit-desc" class="input" rows="2" placeholder="メモ">${task.description || ''}</textarea>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3 mb-4">
                <div>
                  ...
                  <select id="edit-section" class="input">
                    <option value="">なし</option>
                    ${state.sections.map(s => `
                      <option value="${s.id}" ${task.section_id == s.id ? 'selected' : ''}>
                        ${s.icon} ${s.name}
                      </option>
                    `).join('')}
                  </select>
                ...
                <div>
                  <label class="block text-sm text-gray-400 mb-1">期限</label>
                  <input type="date" id="edit-due" class="input" value="${task.due_date || ''}">
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3 mb-4">
                <div>
                  ...
                  <select id="edit-reminder" class="input" onchange="updateEditReminderDay()">
                    <option value="">なし</option>
                    <option value="daily" ${task.reminder_type === 'daily' ? 'selected' : ''}>毎日</option>
                    <option value="weekly" ${task.reminder_type === 'weekly' ? 'selected' : ''}>毎週</option>
                    <option value="monthly" ${task.reminder_type === 'monthly' ? 'selected' : ''}>毎月</option>
                    <option value="monthly_date" ${task.reminder_type === 'monthly_date' ? 'selected' : ''}>毎月○日</option>
                  </select>
                </div>
                <div id="edit-reminder-day-container" class="${task.reminder_type === 'weekly' || task.reminder_type === 'monthly_date' ? '' : 'hidden'}">
                  <label class="block text-sm text-gray-400 mb-1">日/曜日</label>
                  <select id="edit-reminder-day" class="input">
                    ${task.reminder_type === 'weekly' ? 
                      ['日','月','火','水','木','金','土'].map((d, i) => 
                        `<option value="${i}" ${task.reminder_day == i ? 'selected' : ''}>${d}曜日</option>`
                      ).join('') : 
                      task.reminder_type === 'monthly_date' ?
                      Array.from({length: 31}, (_, i) => 
                        `<option value="${i + 1}" ${task.reminder_day == i + 1 ? 'selected' : ''}>${i + 1}日</option>`
                      ).join('') : ''
                    }
                  </select>
                </div>
              ...
              <div class="flex items-center gap-4 mb-4">
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" id="edit-important" class="w-5 h-5 accent-yellow-500" ${task.is_important ? 'checked' : ''}>
...
function renderAttachment(a) {
  const icon = a.type === 'image' ? 'fa-image text-blue-400' :
               a.type === 'pdf' ? 'fa-file-pdf text-red-400' :
               'fa-link text-green-400';
  
  return `
    <div class="attachment-item">
      ${a.type === 'image' ? `<img src="${a.url}" alt="">` : `<i class="fas ${icon}"></i>`}
      <a href="${a.url}" target="_blank" class="truncate flex-1 hover:text-indigo-400">${a.name || a.url}</a>
      <button onclick="deleteAttachment(${a.id})" class="text-gray-500 hover:text-red-400">
        <i class="fas fa-times"></i>
      </button>
    </div>
  `;
